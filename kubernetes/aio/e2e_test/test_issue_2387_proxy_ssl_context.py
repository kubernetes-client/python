...
"""Asyncio counterpart of ``kubernetes/e2e_test/test_issue_2387_proxy_ssl_context.py``.

Same "HTTPS Proxy + HTTPS Destination" scenario as the sync test - a mock
HTTPS forward proxy and a mock HTTPS Kubernetes API server, each with their
own independent self-signed certificate - but exercised through the
aiohttp-based asyncio client. aiohttp gives the proxy's own CONNECT-tunnel
TLS handshake the *same* SSL setting as the destination request
(see ``_ProxySSLContextTCPConnector`` in ``kubernetes/aio/client/rest.py``),
so without that connector override, the proxy's independent CA is never
consulted and the connection fails exactly like the sync client would
without ``proxy_ssl_context``.

Does not require a live Kubernetes cluster - it only requires openssl on
PATH to generate throwaway certificates.
"""
...
@skipUnless(shutil.which("openssl"), "requires openssl on PATH")
class TestProxySslContextAio(IsolatedAsyncioTestCase):
    """Asyncio-client regression coverage for #2387: proxy_ssl_context lets
    the client trust an HTTPS proxy independently of the destination TLS
    settings."""
    ...
    async def test_without_proxy_ssl_context_fails_when_proxy_ca_differs(self):
        # Unlike the sync client's ApiException, aiohttp's own connection
        # errors (raised before the request ever reaches api_client's
        # response handling) surface unwrapped - see rest.py's `request()`,
        # which only catches ApiException around the aiohttp call.
        config = self._make_config()
        async with api_client.ApiClient(config) as client:
    ...
    async def test_with_proxy_ssl_context_succeeds_when_proxy_ca_differs(self):
        config = self._make_config()

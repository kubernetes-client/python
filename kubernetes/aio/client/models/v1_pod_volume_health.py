...
class V1PodVolumeHealth(BaseModel):
    """
    ...
    """ # noqa: E501
    health_conditions: Optional[List[V1VolumeHealthCondition]] = Field(default=None, validation_alias=AliasChoices("healthConditions", "health_conditions"), serialization_alias="healthConditions", description="conditions is the set of adverse conditions reported by the CSI node plugin for this volume on this node. At most 16 conditions may be reported.")
    last_transition_time: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("lastTransitionTime", "last_transition_time"), serialization_alias="lastTransitionTime", description="lastTransitionTime is when the current set of conditions first appeared.")
    name: StrictStr = Field(description="name matches an entry in pod.spec.volumes.")
    openapi_types: ClassVar[Dict[str, str]] = {
        "health_conditions": "List[V1VolumeHealthCondition]",
    ...
    __properties: ClassVar[List[str]] = ["healthConditions", "lastTransitionTime", "name"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "healthConditions" not in obj and "health_conditions" in obj:
            obj["healthConditions"] = obj["health_conditions"]
        obj.pop("health_conditions", None)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, V1PodVolumeHealth):
            return False
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("healthConditions" if serialize else "health_conditions"): _to_legacy_value(getattr(self, "health_conditions", None), serialize),
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.health_conditions:
            for _item_health_conditions in self.health_conditions:
                _items.append(_to_openapi_value(_item_health_conditions) if _item_health_conditions is not None else None)
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "healthConditions": [V1VolumeHealthCondition.from_dict(_item) for _item in obj["healthConditions"]] if obj.get("healthConditions") is not None else None,

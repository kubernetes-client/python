...
class V1AuditAnnotation(BaseModel):
    ...
    value_expression: StrictStr = Field(validation_alias=AliasChoices("valueExpression", "value_expression"), serialization_alias="valueExpression", description="valueExpression represents the expression which is evaluated by CEL to produce an audit annotation value. The expression must evaluate to either a string or null value. If the expression evaluates to a string, the audit annotation is included with the string value. If the expression evaluates to null or empty string the audit annotation will be omitted. The valueExpression may be no longer than 5kb in length. If the result of the valueExpression is more than 10kb in length, it will be truncated to 10kb.  If multiple ValidatingAdmissionPolicyBinding resources match an API request, then the valueExpression will be evaluated for each binding. All unique values produced by the valueExpressions will be joined together in a comma-separated list.  Required.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "valueExpression" not in obj and "value_expression" in obj:
            obj["valueExpression"] = obj["value_expression"]
        obj.pop("value_expression", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("valueExpression" if serialize else "value_expression"): _to_legacy_value(getattr(self, "value_expression", None), serialize),
        }

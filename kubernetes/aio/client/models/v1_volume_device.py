...
class V1VolumeDevice(BaseModel):
    ...
    """ # noqa: E501
    device_path: StrictStr = Field(validation_alias=AliasChoices("devicePath", "device_path"), serialization_alias="devicePath", description="devicePath is the path inside of the container that the device will be mapped to.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "devicePath" not in obj and "device_path" in obj:
            obj["devicePath"] = obj["device_path"]
        obj.pop("device_path", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("devicePath" if serialize else "device_path"): _to_legacy_value(getattr(self, "device_path", None), serialize),

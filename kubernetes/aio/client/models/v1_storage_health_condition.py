...
class V1StorageHealthCondition(BaseModel):
    """
    ...
    """ # noqa: E501
    access_mode: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("accessMode", "access_mode"), serialization_alias="accessMode", description="accessMode is the access mode affected. Nil means all access modes are affected.")
    last_transition_time: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("lastTransitionTime", "last_transition_time"), serialization_alias="lastTransitionTime", description="lastTransitionTime is when this condition first appeared at its current state.")
    message: Optional[StrictStr] = Field(default=None, description="message is a human-readable description. Maximum permitted length of a message is 1024 characters.")
    reason: StrictStr = Field(description="reason is a brief CamelCase machine-parseable reason. Maximum permitted length of a reason is 256 characters.")
    status: StrictStr = Field(description="status is the health status category. One of \"StorageUnreachable\", \"StorageDegraded\".")
    volume_mode: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("volumeMode", "volume_mode"), serialization_alias="volumeMode", description="volumeMode is the volume mode affected. Nil means both are affected.")
    ...
    __properties: ClassVar[List[str]] = ["accessMode", "lastTransitionTime", "message", "reason", "status", "volumeMode"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "accessMode" not in obj and "access_mode" in obj:
            obj["accessMode"] = obj["access_mode"]
        obj.pop("access_mode", None)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, V1StorageHealthCondition):
            return False
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("accessMode" if serialize else "access_mode"): _to_legacy_value(getattr(self, "access_mode", None), serialize),

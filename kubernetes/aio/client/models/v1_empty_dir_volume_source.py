...
class V1EmptyDirVolumeSource(BaseModel):
    ...
    size_limit: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("sizeLimit", "size_limit"), serialization_alias="sizeLimit", description="sizeLimit is the total amount of local storage required for this EmptyDir volume. The size limit is also applicable for memory medium. The maximum usage on memory medium EmptyDir would be the minimum value between the SizeLimit specified here and the sum of memory limits of all containers in a pod. The default is nil which means that the limit is undefined. More info: https://kubernetes.io/docs/concepts/storage/volumes#emptydir")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "sizeLimit" not in obj and "size_limit" in obj:
            obj["sizeLimit"] = obj["size_limit"]
        obj.pop("size_limit", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("medium" if serialize else "medium"): _to_legacy_value(getattr(self, "medium", None), serialize),
            ("mode" if serialize else "mode"): _to_legacy_value(getattr(self, "mode", None), serialize),
            ("sizeLimit" if serialize else "size_limit"): _to_legacy_value(getattr(self, "size_limit", None), serialize),
        }

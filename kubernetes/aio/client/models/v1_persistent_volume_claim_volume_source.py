...
class V1PersistentVolumeClaimVolumeSource(BaseModel):
    ...
    """ # noqa: E501
    claim_name: StrictStr = Field(validation_alias=AliasChoices("claimName", "claim_name"), serialization_alias="claimName", description="claimName is the name of a PersistentVolumeClaim in the same namespace as the pod using this volume. More info: https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistentvolumeclaims")
    read_only: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("readOnly", "read_only"), serialization_alias="readOnly", description="readOnly Will force the ReadOnly setting in VolumeMounts. Default false.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "claimName" not in obj and "claim_name" in obj:
            obj["claimName"] = obj["claim_name"]
        obj.pop("claim_name", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("claimName" if serialize else "claim_name"): _to_legacy_value(getattr(self, "claim_name", None), serialize),

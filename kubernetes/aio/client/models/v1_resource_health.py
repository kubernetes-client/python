...
class V1ResourceHealth(BaseModel):
    ...
    resource_id: StrictStr = Field(validation_alias=AliasChoices("resourceID", "resource_id"), serialization_alias="resourceID", description="ResourceID is the unique identifier of the resource. See the ResourceID type for more information.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "resourceID" not in obj and "resource_id" in obj:
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("health" if serialize else "health"): _to_legacy_value(getattr(self, "health", None), serialize),
            ("message" if serialize else "message"): _to_legacy_value(getattr(self, "message", None), serialize),
            ("resourceID" if serialize else "resource_id"): _to_legacy_value(getattr(self, "resource_id", None), serialize),
        }

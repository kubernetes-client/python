...
class V1CustomResourceSubresourceScale(BaseModel):
    ...
    """ # noqa: E501
    label_selector_path: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("labelSelectorPath", "label_selector_path"), serialization_alias="labelSelectorPath", description="labelSelectorPath defines the JSON path inside of a custom resource that corresponds to Scale `status.selector`. Only JSON paths without the array notation are allowed. Must be a JSON Path under `.status` or `.spec`. Must be set to work with HorizontalPodAutoscaler. The field pointed by this JSON path must be a string field (not a complex selector struct) which contains a serialized label selector in string form. More info: https://kubernetes.io/docs/tasks/access-kubernetes-api/custom-resources/custom-resource-definitions#scale-subresource If there is no value under the given path in the custom resource, the `status.selector` value in the `/scale` subresource will default to the empty string.")
    spec_replicas_path: StrictStr = Field(validation_alias=AliasChoices("specReplicasPath", "spec_replicas_path"), serialization_alias="specReplicasPath", description="specReplicasPath defines the JSON path inside of a custom resource that corresponds to Scale `spec.replicas`. Only JSON paths without the array notation are allowed. Must be a JSON Path under `.spec`. If there is no value under the given path in the custom resource, the `/scale` subresource will return an error on GET.")
    status_replicas_path: StrictStr = Field(validation_alias=AliasChoices("statusReplicasPath", "status_replicas_path"), serialization_alias="statusReplicasPath", description="statusReplicasPath defines the JSON path inside of a custom resource that corresponds to Scale `status.replicas`. Only JSON paths without the array notation are allowed. Must be a JSON Path under `.status`. If there is no value under the given path in the custom resource, the `status.replicas` value in the `/scale` subresource will default to 0.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "labelSelectorPath" not in obj and "label_selector_path" in obj:
            obj["labelSelectorPath"] = obj["label_selector_path"]
        obj.pop("label_selector_path", None)
        if "specReplicasPath" not in obj and "spec_replicas_path" in obj:
            obj["specReplicasPath"] = obj["spec_replicas_path"]
        obj.pop("spec_replicas_path", None)
        if "statusReplicasPath" not in obj and "status_replicas_path" in obj:
            obj["statusReplicasPath"] = obj["status_replicas_path"]
        obj.pop("status_replicas_path", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("labelSelectorPath" if serialize else "label_selector_path"): _to_legacy_value(getattr(self, "label_selector_path", None), serialize),
            ("specReplicasPath" if serialize else "spec_replicas_path"): _to_legacy_value(getattr(self, "spec_replicas_path", None), serialize),
            ("statusReplicasPath" if serialize else "status_replicas_path"): _to_legacy_value(getattr(self, "status_replicas_path", None), serialize),
        }

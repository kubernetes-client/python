...
class V1ServerAddressByClientCIDR(BaseModel):
    ...
    """ # noqa: E501
    client_cidr: StrictStr = Field(validation_alias=AliasChoices("clientCIDR", "client_cidr"), serialization_alias="clientCIDR", description="The CIDR with which clients can match their IP to figure out the server address that they should use.")
    server_address: StrictStr = Field(validation_alias=AliasChoices("serverAddress", "server_address"), serialization_alias="serverAddress", description="Address of this server, suitable for a client that matches the above CIDR. This can be a hostname, hostname:port, IP or IP:port.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "clientCIDR" not in obj and "client_cidr" in obj:
            obj["clientCIDR"] = obj["client_cidr"]
        obj.pop("client_cidr", None)
        if "serverAddress" not in obj and "server_address" in obj:
            obj["serverAddress"] = obj["server_address"]
        obj.pop("server_address", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("clientCIDR" if serialize else "client_cidr"): _to_legacy_value(getattr(self, "client_cidr", None), serialize),
            ("serverAddress" if serialize else "server_address"): _to_legacy_value(getattr(self, "server_address", None), serialize),
        }

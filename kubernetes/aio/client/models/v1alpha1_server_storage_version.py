...
class V1alpha1ServerStorageVersion(BaseModel):
    ...
    """ # noqa: E501
    api_server_id: StrictStr = Field(validation_alias=AliasChoices("apiServerID", "api_server_id"), serialization_alias="apiServerID", description="apiServerID is the ID of the reporting API server.")
    decodable_versions: List[StrictStr] = Field(validation_alias=AliasChoices("decodableVersions", "decodable_versions"), serialization_alias="decodableVersions", description="decodableVersions are the encoding versions the API server can handle to decode. The API server can decode objects encoded in these versions. The encodingVersion must be included in the decodableVersions.")
    encoding_version: StrictStr = Field(validation_alias=AliasChoices("encodingVersion", "encoding_version"), serialization_alias="encodingVersion", description="encodingVersion the API server encodes the object to when persisting it in the backend (e.g., etcd).")
    served_versions: Optional[List[StrictStr]] = Field(default=None, validation_alias=AliasChoices("servedVersions", "served_versions"), serialization_alias="servedVersions", description="servedVersions lists all versions the API server can serve. DecodableVersions must include all ServedVersions.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "apiServerID" not in obj and "api_server_id" in obj:
            obj["apiServerID"] = obj["api_server_id"]
        obj.pop("api_server_id", None)
        if "decodableVersions" not in obj and "decodable_versions" in obj:
            obj["decodableVersions"] = obj["decodable_versions"]
        obj.pop("decodable_versions", None)
        if "encodingVersion" not in obj and "encoding_version" in obj:
            obj["encodingVersion"] = obj["encoding_version"]
        obj.pop("encoding_version", None)
        if "servedVersions" not in obj and "served_versions" in obj:
            obj["servedVersions"] = obj["served_versions"]
        obj.pop("served_versions", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("apiServerID" if serialize else "api_server_id"): _to_legacy_value(getattr(self, "api_server_id", None), serialize),
            ("decodableVersions" if serialize else "decodable_versions"): _to_legacy_value(getattr(self, "decodable_versions", None), serialize),
            ("encodingVersion" if serialize else "encoding_version"): _to_legacy_value(getattr(self, "encoding_version", None), serialize),
            ("servedVersions" if serialize else "served_versions"): _to_legacy_value(getattr(self, "served_versions", None), serialize),
        }

...
class V2MetricTarget(BaseModel):
    ...
    """ # noqa: E501
    average_utilization: Optional[StrictInt] = Field(default=None, validation_alias=AliasChoices("averageUtilization", "average_utilization"), serialization_alias="averageUtilization", description="averageUtilization is the target value of the average of the resource metric across all relevant pods, represented as a percentage of the requested value of the resource for the pods. Currently only valid for Resource metric source type")
    average_value: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("averageValue", "average_value"), serialization_alias="averageValue", description="averageValue is the target value of the average of the metric across all relevant pods (as a quantity)")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "averageUtilization" not in obj and "average_utilization" in obj:
            obj["averageUtilization"] = obj["average_utilization"]
        obj.pop("average_utilization", None)
        if "averageValue" not in obj and "average_value" in obj:
            obj["averageValue"] = obj["average_value"]
        obj.pop("average_value", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("averageUtilization" if serialize else "average_utilization"): _to_legacy_value(getattr(self, "average_utilization", None), serialize),
            ("averageValue" if serialize else "average_value"): _to_legacy_value(getattr(self, "average_value", None), serialize),

...
class V1alpha3ShareableCapacityStatus(BaseModel):
    """
    ...
    """ # noqa: E501
    available: StrictStr = Field(description="Available is Total minus Consumed, never negative.")
    consumed: StrictStr = Field(description="Consumed is the amount drawn by current allocations.")
    name: StrictStr = Field(description="Name is the capacity name.")
    total: StrictStr = Field(description="Total is the sum of this capacity across shareable devices in the pool.")
    ...
    __properties: ClassVar[List[str]] = ["available", "consumed", "name", "total"]
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, V1alpha3ShareableCapacityStatus):
            return False
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("available" if serialize else "available"): _to_legacy_value(getattr(self, "available", None), serialize),
            ("consumed" if serialize else "consumed"): _to_legacy_value(getattr(self, "consumed", None), serialize),

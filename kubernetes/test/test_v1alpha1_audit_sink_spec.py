# coding: utf-8

"""
    Kubernetes

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    OpenAPI spec version: release-1.15
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest

import client
from client.models.v1alpha1_audit_sink_spec import V1alpha1AuditSinkSpec  # noqa: E501
from client.rest import ApiException


class TestV1alpha1AuditSinkSpec(unittest.TestCase):
    """V1alpha1AuditSinkSpec unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testV1alpha1AuditSinkSpec(self):
        """Test V1alpha1AuditSinkSpec"""
        # FIXME: construct object with mandatory attributes with example values
        # model = client.models.v1alpha1_audit_sink_spec.V1alpha1AuditSinkSpec()  # noqa: E501
        pass


if __name__ == '__main__':
    unittest.main()

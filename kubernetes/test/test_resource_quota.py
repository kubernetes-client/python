...
class TestResourceQuotasEqual(unittest.TestCase):
    ...
    def test_differing_scopes_returns_false(self):
        """Quotas with different scopes are not equal despite equal hard."""
        spec_a = client.V1ResourceQuotaSpec(hard={"cpu": "1"}, scopes=["BestEffort"])
        spec_b = client.V1ResourceQuotaSpec(hard={"cpu": "1"}, scopes=["NotBestEffort"])
        quota_a = client.V1ResourceQuota(spec=spec_a)
        quota_b = client.V1ResourceQuota(spec=spec_b)
        self.assertFalse(resource_quotas_equal(quota_a, quota_b))

    def test_same_scopes_different_order_returns_true(self):
        """Scope list order does not affect equality."""
        spec_a = client.V1ResourceQuotaSpec(
            hard={"cpu": "1"}, scopes=["BestEffort", "NotTerminating"]
        )
        spec_b = client.V1ResourceQuotaSpec(
            hard={"cpu": "1"}, scopes=["NotTerminating", "BestEffort"]
        )
        quota_a = client.V1ResourceQuota(spec=spec_a)
        quota_b = client.V1ResourceQuota(spec=spec_b)
        self.assertTrue(resource_quotas_equal(quota_a, quota_b))

    def test_differing_scope_selector_returns_false(self):
        """Quotas with different scope selectors are not equal."""
        selector_a = client.V1ScopeSelector(
            match_expressions=[
        ...
        selector_b = client.V1ScopeSelector(
            match_expressions=[
                client.V1ScopedResourceSelectorRequirement(
                    scope_name="PriorityClass",
                    operator="In",
        ...
        spec_a = client.V1ResourceQuotaSpec(hard={"cpu": "1"}, scope_selector=selector_a)
        spec_b = client.V1ResourceQuotaSpec(hard={"cpu": "1"}, scope_selector=selector_b)
        quota_a = client.V1ResourceQuota(spec=spec_a)
        quota_b = client.V1ResourceQuota(spec=spec_b)
        self.assertFalse(resource_quotas_equal(quota_a, quota_b))

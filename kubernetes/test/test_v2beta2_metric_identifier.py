# coding: utf-8

"""
    Kubernetes

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    OpenAPI spec version: release-1.15
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest

import client
from client.models.v2beta2_metric_identifier import V2beta2MetricIdentifier  # noqa: E501
from client.rest import ApiException


class TestV2beta2MetricIdentifier(unittest.TestCase):
    """V2beta2MetricIdentifier unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testV2beta2MetricIdentifier(self):
        """Test V2beta2MetricIdentifier"""
        # FIXME: construct object with mandatory attributes with example values
        # model = client.models.v2beta2_metric_identifier.V2beta2MetricIdentifier()  # noqa: E501
        pass


if __name__ == '__main__':
    unittest.main()

...
class V1CronJobStatus(BaseModel):
    ...
    last_schedule_time: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("lastScheduleTime", "last_schedule_time"), serialization_alias="lastScheduleTime", description="Information when was the last time the job was successfully scheduled.")
    last_successful_time: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("lastSuccessfulTime", "last_successful_time"), serialization_alias="lastSuccessfulTime", description="Information when was the last time the job successfully completed.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "lastScheduleTime" not in obj and "last_schedule_time" in obj:
            obj["lastScheduleTime"] = obj["last_schedule_time"]
        obj.pop("last_schedule_time", None)
        if "lastSuccessfulTime" not in obj and "last_successful_time" in obj:
            obj["lastSuccessfulTime"] = obj["last_successful_time"]
        obj.pop("last_successful_time", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("lastScheduleTime" if serialize else "last_schedule_time"): _to_legacy_value(getattr(self, "last_schedule_time", None), serialize),
            ("lastSuccessfulTime" if serialize else "last_successful_time"): _to_legacy_value(getattr(self, "last_successful_time", None), serialize),
        }

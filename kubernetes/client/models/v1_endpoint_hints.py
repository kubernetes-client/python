...
class V1EndpointHints(BaseModel):
    ...
    """ # noqa: E501
    for_nodes: Optional[List[V1ForNode]] = Field(default=None, validation_alias=AliasChoices("forNodes", "for_nodes"), serialization_alias="forNodes", description="forNodes indicates the node(s) this endpoint should be consumed by when using topology aware routing. May contain a maximum of 8 entries.")
    for_zones: Optional[List[V1ForZone]] = Field(default=None, validation_alias=AliasChoices("forZones", "for_zones"), serialization_alias="forZones", description="forZones indicates the zone(s) this endpoint should be consumed by when using topology aware routing. May contain a maximum of 8 entries.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "forNodes" not in obj and "for_nodes" in obj:
            obj["forNodes"] = obj["for_nodes"]
        obj.pop("for_nodes", None)
        if "forZones" not in obj and "for_zones" in obj:
            obj["forZones"] = obj["for_zones"]
        obj.pop("for_zones", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("forNodes" if serialize else "for_nodes"): _to_legacy_value(getattr(self, "for_nodes", None), serialize),
            ("forZones" if serialize else "for_zones"): _to_legacy_value(getattr(self, "for_zones", None), serialize),
        }

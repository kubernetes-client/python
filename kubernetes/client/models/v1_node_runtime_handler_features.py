...
class V1NodeRuntimeHandlerFeatures(BaseModel):
    ...
    """ # noqa: E501
    recursive_read_only_mounts: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("recursiveReadOnlyMounts", "recursive_read_only_mounts"), serialization_alias="recursiveReadOnlyMounts", description="RecursiveReadOnlyMounts is set to true if the runtime handler supports RecursiveReadOnlyMounts.")
    user_namespaces: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("userNamespaces", "user_namespaces"), serialization_alias="userNamespaces", description="UserNamespaces is set to true if the runtime handler supports UserNamespaces, including for volumes.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "recursiveReadOnlyMounts" not in obj and "recursive_read_only_mounts" in obj:
            obj["recursiveReadOnlyMounts"] = obj["recursive_read_only_mounts"]
        obj.pop("recursive_read_only_mounts", None)
        if "userNamespaces" not in obj and "user_namespaces" in obj:
            obj["userNamespaces"] = obj["user_namespaces"]
        obj.pop("user_namespaces", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("recursiveReadOnlyMounts" if serialize else "recursive_read_only_mounts"): _to_legacy_value(getattr(self, "recursive_read_only_mounts", None), serialize),
            ("userNamespaces" if serialize else "user_namespaces"): _to_legacy_value(getattr(self, "user_namespaces", None), serialize),
        }

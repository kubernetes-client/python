...
class V1CustomResourceDefinitionNames(BaseModel):
    ...
    list_kind: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("listKind", "list_kind"), serialization_alias="listKind", description="listKind is the serialized kind of the list for this resource. Defaults to \"`kind`List\".")
    ...
    short_names: Optional[List[StrictStr]] = Field(default=None, validation_alias=AliasChoices("shortNames", "short_names"), serialization_alias="shortNames", description="shortNames are short names for the resource, exposed in API discovery documents, and used by clients to support invocations like `kubectl get <shortname>`. It must be all lowercase.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "listKind" not in obj and "list_kind" in obj:
            obj["listKind"] = obj["list_kind"]
        obj.pop("list_kind", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("listKind" if serialize else "list_kind"): _to_legacy_value(getattr(self, "list_kind", None), serialize),
            ("plural" if serialize else "plural"): _to_legacy_value(getattr(self, "plural", None), serialize),
            ("shortNames" if serialize else "short_names"): _to_legacy_value(getattr(self, "short_names", None), serialize),
            ("singular" if serialize else "singular"): _to_legacy_value(getattr(self, "singular", None), serialize),
        }

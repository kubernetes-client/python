...
class V1AzureFilePersistentVolumeSource(BaseModel):
    ...
    secret_name: StrictStr = Field(validation_alias=AliasChoices("secretName", "secret_name"), serialization_alias="secretName", description="secretName is the name of secret that contains Azure Storage Account Name and Key")
    secret_namespace: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("secretNamespace", "secret_namespace"), serialization_alias="secretNamespace", description="secretNamespace is the namespace of the secret that contains Azure Storage Account Name and Key default is the same as the Pod")
    share_name: StrictStr = Field(validation_alias=AliasChoices("shareName", "share_name"), serialization_alias="shareName", description="shareName is the azure Share Name")
    ...
    def __preprocess_input_names(
        ...
        if "secretName" not in obj and "secret_name" in obj:
            obj["secretName"] = obj["secret_name"]
        obj.pop("secret_name", None)
        if "secretNamespace" not in obj and "secret_namespace" in obj:
            obj["secretNamespace"] = obj["secret_namespace"]
        obj.pop("secret_namespace", None)
        if "shareName" not in obj and "share_name" in obj:
            obj["shareName"] = obj["share_name"]
        obj.pop("share_name", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("secretName" if serialize else "secret_name"): _to_legacy_value(getattr(self, "secret_name", None), serialize),
            ("secretNamespace" if serialize else "secret_namespace"): _to_legacy_value(getattr(self, "secret_namespace", None), serialize),
            ("shareName" if serialize else "share_name"): _to_legacy_value(getattr(self, "share_name", None), serialize),
        }

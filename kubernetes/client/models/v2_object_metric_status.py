...
class V2ObjectMetricStatus(BaseModel):
    ...
    described_object: V2CrossVersionObjectReference = Field(validation_alias=AliasChoices("describedObject", "described_object"), serialization_alias="describedObject")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "describedObject" not in obj and "described_object" in obj:
            obj["describedObject"] = obj["described_object"]
        obj.pop("described_object", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("current" if serialize else "current"): _to_legacy_value(getattr(self, "current", None), serialize),
            ("describedObject" if serialize else "described_object"): _to_legacy_value(getattr(self, "described_object", None), serialize),
            ("metric" if serialize else "metric"): _to_legacy_value(getattr(self, "metric", None), serialize),
        }

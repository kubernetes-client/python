...
class V1CSINodeDriver(BaseModel):
    ...
    node_id: StrictStr = Field(validation_alias=AliasChoices("nodeID", "node_id"), serialization_alias="nodeID", description="nodeID of the node from the driver point of view. This field enables Kubernetes to communicate with storage systems that do not share the same nomenclature for nodes. For example, Kubernetes may refer to a given node as \"node1\", but the storage system may refer to the same node as \"nodeA\". When Kubernetes issues a command to the storage system to attach a volume to a specific node, it can use this field to refer to the node name using the ID that the storage system will understand, e.g. \"nodeA\" instead of \"node1\". This field is required.")
    topology_keys: Optional[List[StrictStr]] = Field(default=None, validation_alias=AliasChoices("topologyKeys", "topology_keys"), serialization_alias="topologyKeys", description="topologyKeys is the list of keys supported by the driver. When a driver is initialized on a cluster, it provides a set of topology keys that it understands (e.g. \"company.com/zone\", \"company.com/region\"). When a driver is initialized on a node, it provides the same topology keys along with values. Kubelet will expose these topology keys as labels on its own node object. When Kubernetes does topology aware provisioning, it can use this list to determine which labels it should retrieve from the node object and pass back to the driver. It is possible for different nodes to use different topology keys. This can be empty if driver does not support topology.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "nodeID" not in obj and "node_id" in obj:
            obj["nodeID"] = obj["node_id"]
        obj.pop("node_id", None)
        if "topologyKeys" not in obj and "topology_keys" in obj:
            obj["topologyKeys"] = obj["topology_keys"]
        obj.pop("topology_keys", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("nodeID" if serialize else "node_id"): _to_legacy_value(getattr(self, "node_id", None), serialize),
            ("topologyKeys" if serialize else "topology_keys"): _to_legacy_value(getattr(self, "topology_keys", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.allocatable:
            _dict['allocatable'] = _to_openapi_value(self.allocatable)
        return _dict

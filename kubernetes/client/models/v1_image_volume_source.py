...
class V1ImageVolumeSource(BaseModel):
    ...
    """ # noqa: E501
    pull_policy: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("pullPolicy", "pull_policy"), serialization_alias="pullPolicy", description="Policy for pulling OCI objects. Possible values are: Always: the kubelet always attempts to pull the reference. Container creation will fail If the pull fails. Never: the kubelet never pulls the reference and only uses a local image or artifact. Container creation will fail if the reference isn't present. IfNotPresent: the kubelet pulls if the reference isn't already present on disk. Container creation will fail if the reference isn't present and the pull fails. Defaults to Always if :latest tag is specified, or IfNotPresent otherwise.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "pullPolicy" not in obj and "pull_policy" in obj:
            obj["pullPolicy"] = obj["pull_policy"]
        obj.pop("pull_policy", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("pullPolicy" if serialize else "pull_policy"): _to_legacy_value(getattr(self, "pull_policy", None), serialize),
            ("reference" if serialize else "reference"): _to_legacy_value(getattr(self, "reference", None), serialize),
        }

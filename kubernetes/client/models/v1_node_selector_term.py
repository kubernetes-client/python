...
class V1NodeSelectorTerm(BaseModel):
    ...
    """ # noqa: E501
    match_expressions: Optional[List[V1NodeSelectorRequirement]] = Field(default=None, validation_alias=AliasChoices("matchExpressions", "match_expressions"), serialization_alias="matchExpressions", description="A list of node selector requirements by node's labels.")
    match_fields: Optional[List[V1NodeSelectorRequirement]] = Field(default=None, validation_alias=AliasChoices("matchFields", "match_fields"), serialization_alias="matchFields", description="A list of node selector requirements by node's fields.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "matchExpressions" not in obj and "match_expressions" in obj:
            obj["matchExpressions"] = obj["match_expressions"]
        obj.pop("match_expressions", None)
        if "matchFields" not in obj and "match_fields" in obj:
            obj["matchFields"] = obj["match_fields"]
        obj.pop("match_fields", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("matchExpressions" if serialize else "match_expressions"): _to_legacy_value(getattr(self, "match_expressions", None), serialize),
            ("matchFields" if serialize else "match_fields"): _to_legacy_value(getattr(self, "match_fields", None), serialize),
        }

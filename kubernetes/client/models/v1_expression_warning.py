...
class V1ExpressionWarning(BaseModel):
    ...
    """ # noqa: E501
    field_ref: StrictStr = Field(validation_alias=AliasChoices("fieldRef", "field_ref"), serialization_alias="fieldRef", description="fieldRef is the path to the field that refers to the expression. For example, the reference to the expression of the first item of validations is \"spec.validations[0].expression\"")
    warning: StrictStr = Field(description="warning contains the content of type checking information in a human-readable form. Each line of the warning contains the type that the expression is checked against, followed by the type check error from the compiler.")
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("warning" if serialize else "warning"): _to_legacy_value(getattr(self, "warning", None), serialize),
        }

...
class V1NetworkPolicyPort(BaseModel):
    ...
    """ # noqa: E501
    end_port: Optional[StrictInt] = Field(default=None, validation_alias=AliasChoices("endPort", "end_port"), serialization_alias="endPort", description="endPort indicates that the range of ports from port to endPort if set, inclusive, should be allowed by the policy. This field cannot be defined if the port field is not defined or if the port field is defined as a named (string) port. The endPort must be equal or greater than port.")
    port: Optional[StrictInt | StrictStr] = Field(default=None, description="port represents the port on the given protocol. This can either be a numerical or named port on a pod. If this field is not provided, this matches all port names and numbers. If present, only traffic on the specified protocol AND port will be matched.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "endPort" not in obj and "end_port" in obj:
            obj["endPort"] = obj["end_port"]
        obj.pop("end_port", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("endPort" if serialize else "end_port"): _to_legacy_value(getattr(self, "end_port", None), serialize),

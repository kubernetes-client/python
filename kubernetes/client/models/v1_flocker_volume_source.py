...
class V1FlockerVolumeSource(BaseModel):
    ...
    """ # noqa: E501
    dataset_name: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("datasetName", "dataset_name"), serialization_alias="datasetName", description="datasetName is Name of the dataset stored as metadata -> name on the dataset for Flocker should be considered as deprecated")
    dataset_uuid: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("datasetUUID", "dataset_uuid"), serialization_alias="datasetUUID", description="datasetUUID is the UUID of the dataset. This is unique identifier of a Flocker dataset")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "datasetName" not in obj and "dataset_name" in obj:
            obj["datasetName"] = obj["dataset_name"]
        obj.pop("dataset_name", None)
        if "datasetUUID" not in obj and "dataset_uuid" in obj:
            obj["datasetUUID"] = obj["dataset_uuid"]
        obj.pop("dataset_uuid", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("datasetName" if serialize else "dataset_name"): _to_legacy_value(getattr(self, "dataset_name", None), serialize),
            ("datasetUUID" if serialize else "dataset_uuid"): _to_legacy_value(getattr(self, "dataset_uuid", None), serialize),
        }

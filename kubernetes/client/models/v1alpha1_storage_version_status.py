...
class V1alpha1StorageVersionStatus(BaseModel):
    ...
    """ # noqa: E501
    common_encoding_version: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("commonEncodingVersion", "common_encoding_version"), serialization_alias="commonEncodingVersion", description="commonEncodingVersion is set to an encoding storage version if all API server instances share that same version. If they don't share one storage version, this field is left empty. API servers should finish updating its storageVersionStatus entry before serving write operations, so that this field will be in sync with the reality.")
    conditions: Optional[List[V1alpha1StorageVersionCondition]] = Field(default=None, description="conditions lists the latest available observations of the storageVersion's state.")
    storage_versions: Optional[List[V1alpha1ServerStorageVersion]] = Field(default=None, validation_alias=AliasChoices("storageVersions", "storage_versions"), serialization_alias="storageVersions", description="storageVersions lists the reported versions per API server instance.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "commonEncodingVersion" not in obj and "common_encoding_version" in obj:
            obj["commonEncodingVersion"] = obj["common_encoding_version"]
        obj.pop("common_encoding_version", None)
        if "storageVersions" not in obj and "storage_versions" in obj:
            obj["storageVersions"] = obj["storage_versions"]
        obj.pop("storage_versions", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("commonEncodingVersion" if serialize else "common_encoding_version"): _to_legacy_value(getattr(self, "common_encoding_version", None), serialize),
            ("conditions" if serialize else "conditions"): _to_legacy_value(getattr(self, "conditions", None), serialize),
            ("storageVersions" if serialize else "storage_versions"): _to_legacy_value(getattr(self, "storage_versions", None), serialize),
        }

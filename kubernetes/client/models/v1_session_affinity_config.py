...
class V1SessionAffinityConfig(BaseModel):
    ...
    """ # noqa: E501
    client_ip: Optional[V1ClientIPConfig] = Field(default=None, validation_alias=AliasChoices("clientIP", "client_ip"), serialization_alias="clientIP")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "clientIP" not in obj and "client_ip" in obj:
            obj["clientIP"] = obj["client_ip"]
        obj.pop("client_ip", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("clientIP" if serialize else "client_ip"): _to_legacy_value(getattr(self, "client_ip", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.client_ip:
            _dict['clientIP'] = _to_openapi_value(self.client_ip)
        return _dict

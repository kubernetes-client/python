...
class V1alpha3ResourcePoolStatusRequestSpec(BaseModel):
    """
    ...
    """ # noqa: E501
    default_partition_type_attribute: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("defaultPartitionTypeAttribute", "default_partition_type_attribute"), serialization_alias="defaultPartitionTypeAttribute", description="DefaultPartitionTypeAttribute optionally names a device attribute (by its fully qualified name, e.g. \"gpu.example.com/profile\") to use as the default grouping attribute for partitionable devices whose slice has not declared one themselves.  A slice's own PartitionTypeAttribute always takes precedence. This default applies only to devices whose slice does not declare one, so that a request can still get an accurate partitionSummary from a driver that has not been updated to declare it. When neither the slice nor this default names an attribute, a partitionable pool reports no partitionSummary.  Must include the domain qualifier.")
    driver: StrictStr = Field(description="Driver specifies the DRA driver name to filter pools. Only pools from ResourceSlices with this driver will be included. Must be a DNS subdomain (e.g., \"gpu.example.com\").")
    limit: Optional[StrictInt] = Field(default=None, description="Limit optionally specifies the maximum number of pools to return in the status. If more pools match the filter criteria, the response will be truncated (i.e., len(status.pools) < status.poolCount).  Default: 100 Minimum: 1 Maximum: 1000")
    pool_name: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("poolName", "pool_name"), serialization_alias="poolName", description="PoolName optionally filters to a specific pool name. If not specified, all pools from the specified driver are included. When specified, must be a non-empty valid resource pool name (DNS subdomains separated by \"/\").")
    openapi_types: ClassVar[Dict[str, str]] = {
        "default_partition_type_attribute": "str",
        "driver": "str",
        "limit": "int",
        "pool_name": "str"
    }
    ...
    attribute_map: ClassVar[Dict[str, str]] = {
        "default_partition_type_attribute": "defaultPartitionTypeAttribute",
        "driver": "driver",
        "limit": "limit",
        "pool_name": "poolName"
    }
    __properties: ClassVar[List[str]] = ["defaultPartitionTypeAttribute", "driver", "limit", "poolName"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "defaultPartitionTypeAttribute" not in obj and "default_partition_type_attribute" in obj:
            obj["defaultPartitionTypeAttribute"] = obj["default_partition_type_attribute"]
        obj.pop("default_partition_type_attribute", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("defaultPartitionTypeAttribute" if serialize else "default_partition_type_attribute"): _to_legacy_value(getattr(self, "default_partition_type_attribute", None), serialize),
            ("driver" if serialize else "driver"): _to_legacy_value(getattr(self, "driver", None), serialize),
            ("limit" if serialize else "limit"): _to_legacy_value(getattr(self, "limit", None), serialize),
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "defaultPartitionTypeAttribute": obj.get("defaultPartitionTypeAttribute"),

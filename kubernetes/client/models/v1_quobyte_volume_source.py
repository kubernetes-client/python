...
class V1QuobyteVolumeSource(BaseModel):
    ...
    read_only: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("readOnly", "read_only"), serialization_alias="readOnly", description="readOnly here will force the Quobyte volume to be mounted with read-only permissions. Defaults to false.")
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("registry" if serialize else "registry"): _to_legacy_value(getattr(self, "registry", None), serialize),
            ("tenant" if serialize else "tenant"): _to_legacy_value(getattr(self, "tenant", None), serialize),
            ("user" if serialize else "user"): _to_legacy_value(getattr(self, "user", None), serialize),
            ("volume" if serialize else "volume"): _to_legacy_value(getattr(self, "volume", None), serialize),
        }

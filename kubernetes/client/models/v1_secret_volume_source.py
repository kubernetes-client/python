...
class V1SecretVolumeSource(BaseModel):
    ...
    secret_name: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("secretName", "secret_name"), serialization_alias="secretName", description="secretName is the name of the secret in the pod's namespace to use. More info: https://kubernetes.io/docs/concepts/storage/volumes#secret")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "defaultMode" not in obj and "default_mode" in obj:
            obj["defaultMode"] = obj["default_mode"]
        obj.pop("default_mode", None)
        if "defaultUser" not in obj and "default_user" in obj:
            obj["defaultUser"] = obj["default_user"]
        obj.pop("default_user", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("defaultMode" if serialize else "default_mode"): _to_legacy_value(getattr(self, "default_mode", None), serialize),
            ("defaultUser" if serialize else "default_user"): _to_legacy_value(getattr(self, "default_user", None), serialize),

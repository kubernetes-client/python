...
class V1AggregationRule(BaseModel):
    ...
    """ # noqa: E501
    cluster_role_selectors: Optional[List[V1LabelSelector]] = Field(default=None, validation_alias=AliasChoices("clusterRoleSelectors", "cluster_role_selectors"), serialization_alias="clusterRoleSelectors", description="ClusterRoleSelectors holds a list of selectors which will be used to find ClusterRoles and create the rules. If any of the selectors match, then the ClusterRole's permissions will be added")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "clusterRoleSelectors" not in obj and "cluster_role_selectors" in obj:
            obj["clusterRoleSelectors"] = obj["cluster_role_selectors"]
        obj.pop("cluster_role_selectors", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("clusterRoleSelectors" if serialize else "cluster_role_selectors"): _to_legacy_value(getattr(self, "cluster_role_selectors", None), serialize),
        }

...
class V1NodeConfigStatus(BaseModel):
    ...
    last_known_good: Optional[V1NodeConfigSource] = Field(default=None, validation_alias=AliasChoices("lastKnownGood", "last_known_good"), serialization_alias="lastKnownGood")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "lastKnownGood" not in obj and "last_known_good" in obj:
            obj["lastKnownGood"] = obj["last_known_good"]
        obj.pop("last_known_good", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("assigned" if serialize else "assigned"): _to_legacy_value(getattr(self, "assigned", None), serialize),
            ("error" if serialize else "error"): _to_legacy_value(getattr(self, "error", None), serialize),
            ("lastKnownGood" if serialize else "last_known_good"): _to_legacy_value(getattr(self, "last_known_good", None), serialize),
        }

...
class V1ResourceQuotaSpec(BaseModel):
    ...
    scope_selector: Optional[V1ScopeSelector] = Field(default=None, validation_alias=AliasChoices("scopeSelector", "scope_selector"), serialization_alias="scopeSelector")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "scopeSelector" not in obj and "scope_selector" in obj:
            obj["scopeSelector"] = obj["scope_selector"]
        obj.pop("scope_selector", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("hard" if serialize else "hard"): _to_legacy_value(getattr(self, "hard", None), serialize),
            ("scopeSelector" if serialize else "scope_selector"): _to_legacy_value(getattr(self, "scope_selector", None), serialize),
            ("scopes" if serialize else "scopes"): _to_legacy_value(getattr(self, "scopes", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.scope_selector:
            _dict['scopeSelector'] = _to_openapi_value(self.scope_selector)
        return _dict

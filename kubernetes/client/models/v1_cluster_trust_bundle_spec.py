...
class V1ClusterTrustBundleSpec(BaseModel):
    """
    ...
    """ # noqa: E501
    signer_name: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("signerName", "signer_name"), serialization_alias="signerName", description="signerName indicates the associated signer, if any.  In order to create or update a ClusterTrustBundle that sets signerName, you must have the following cluster-scoped permission: group=certificates.k8s.io resource=signers resourceName=<the signer name> verb=attest.  If signerName is not empty, then the ClusterTrustBundle object must be named with the signer name as a prefix (translating slashes to colons). For example, for the signer name `example.com/foo`, valid ClusterTrustBundle object names include `example.com:foo:abc` and `example.com:foo:v1`.  If signerName is empty, then the ClusterTrustBundle object's name must not have such a prefix.  List/watch requests for ClusterTrustBundles can filter on this field using a `spec.signerName=NAME` field selector.")
    trust_bundle: StrictStr = Field(validation_alias=AliasChoices("trustBundle", "trust_bundle"), serialization_alias="trustBundle", description="trustBundle contains the individual X.509 trust anchors for this bundle, as PEM bundle of PEM-wrapped, DER-formatted X.509 certificates.  The data must consist only of PEM certificate blocks that parse as valid X.509 certificates.  Each certificate must include a basic constraints extension with the CA bit set.  The API server will reject objects that contain duplicate certificates, or that use PEM block headers.  Users of ClusterTrustBundles, including Kubelet, are free to reorder and deduplicate certificate blocks in this file according to their own logic, as well as to drop PEM block headers and inter-block data.")
    ...
    def __preprocess_input_names(
        ...
        if "trustBundle" not in obj and "trust_bundle" in obj:
            obj["trustBundle"] = obj["trust_bundle"]
        obj.pop("trust_bundle", None)
        return obj
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, V1ClusterTrustBundleSpec):
            return False
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("trustBundle" if serialize else "trust_bundle"): _to_legacy_value(getattr(self, "trust_bundle", None), serialize),
        }

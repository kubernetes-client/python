...
class V1VolumeAttachmentSource(BaseModel):
    ...
    """ # noqa: E501
    inline_volume_spec: Optional[V1PersistentVolumeSpec] = Field(default=None, validation_alias=AliasChoices("inlineVolumeSpec", "inline_volume_spec"), serialization_alias="inlineVolumeSpec")
    persistent_volume_name: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("persistentVolumeName", "persistent_volume_name"), serialization_alias="persistentVolumeName", description="persistentVolumeName represents the name of the persistent volume to attach.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "inlineVolumeSpec" not in obj and "inline_volume_spec" in obj:
            obj["inlineVolumeSpec"] = obj["inline_volume_spec"]
        obj.pop("inline_volume_spec", None)
        if "persistentVolumeName" not in obj and "persistent_volume_name" in obj:
            obj["persistentVolumeName"] = obj["persistent_volume_name"]
        obj.pop("persistent_volume_name", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("inlineVolumeSpec" if serialize else "inline_volume_spec"): _to_legacy_value(getattr(self, "inline_volume_spec", None), serialize),
            ("persistentVolumeName" if serialize else "persistent_volume_name"): _to_legacy_value(getattr(self, "persistent_volume_name", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.inline_volume_spec:
            _dict['inlineVolumeSpec'] = _to_openapi_value(self.inline_volume_spec)
        return _dict

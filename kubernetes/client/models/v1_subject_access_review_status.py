...
class V1SubjectAccessReviewStatus(BaseModel):
    ...
    evaluation_error: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("evaluationError", "evaluation_error"), serialization_alias="evaluationError", description="evaluationError is an indication that some error occurred during the authorization check. It is entirely possible to get an error and be able to continue determine authorization status in spite of it. For instance, RBAC can be missing a role, but enough roles are still present and bound to reason about the request.")
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("allowed" if serialize else "allowed"): _to_legacy_value(getattr(self, "allowed", None), serialize),
            ("denied" if serialize else "denied"): _to_legacy_value(getattr(self, "denied", None), serialize),

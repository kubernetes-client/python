...
class V1TokenRequestStatus(BaseModel):
    ...
    """ # noqa: E501
    expiration_timestamp: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("expirationTimestamp", "expiration_timestamp"), serialization_alias="expirationTimestamp", description="expirationTimestamp is the time of expiration of the returned token.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "expirationTimestamp" not in obj and "expiration_timestamp" in obj:
            obj["expirationTimestamp"] = obj["expiration_timestamp"]
        obj.pop("expiration_timestamp", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("expirationTimestamp" if serialize else "expiration_timestamp"): _to_legacy_value(getattr(self, "expiration_timestamp", None), serialize),
            ("token" if serialize else "token"): _to_legacy_value(getattr(self, "token", None), serialize),
        }

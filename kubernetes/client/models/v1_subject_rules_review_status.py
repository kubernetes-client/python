...
class V1SubjectRulesReviewStatus(BaseModel):
    ...
    """ # noqa: E501
    evaluation_error: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("evaluationError", "evaluation_error"), serialization_alias="evaluationError", description="evaluationError can appear in combination with Rules. It indicates an error occurred during rule evaluation, such as an authorizer that doesn't support rule evaluation, and that ResourceRules and/or NonResourceRules may be incomplete.")
    ...
    non_resource_rules: List[V1NonResourceRule] = Field(validation_alias=AliasChoices("nonResourceRules", "non_resource_rules"), serialization_alias="nonResourceRules", description="nonResourceRules is the list of actions the subject is allowed to perform on non-resources. The list ordering isn't significant, may contain duplicates, and possibly be incomplete.")
    resource_rules: List[V1ResourceRule] = Field(validation_alias=AliasChoices("resourceRules", "resource_rules"), serialization_alias="resourceRules", description="resourceRules is the list of actions the subject is allowed to perform on resources. The list ordering isn't significant, may contain duplicates, and possibly be incomplete.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "evaluationError" not in obj and "evaluation_error" in obj:
            obj["evaluationError"] = obj["evaluation_error"]
        obj.pop("evaluation_error", None)
        if "nonResourceRules" not in obj and "non_resource_rules" in obj:
            obj["nonResourceRules"] = obj["non_resource_rules"]
        obj.pop("non_resource_rules", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("evaluationError" if serialize else "evaluation_error"): _to_legacy_value(getattr(self, "evaluation_error", None), serialize),
            ("incomplete" if serialize else "incomplete"): _to_legacy_value(getattr(self, "incomplete", None), serialize),
            ("nonResourceRules" if serialize else "non_resource_rules"): _to_legacy_value(getattr(self, "non_resource_rules", None), serialize),

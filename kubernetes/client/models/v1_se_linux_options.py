...
class V1SELinuxOptions(BaseModel):
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("level" if serialize else "level"): _to_legacy_value(getattr(self, "level", None), serialize),
            ("role" if serialize else "role"): _to_legacy_value(getattr(self, "role", None), serialize),

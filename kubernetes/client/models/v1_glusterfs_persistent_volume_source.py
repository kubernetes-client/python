...
class V1GlusterfsPersistentVolumeSource(BaseModel):
    ...
    endpoints_namespace: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("endpointsNamespace", "endpoints_namespace"), serialization_alias="endpointsNamespace", description="endpointsNamespace is the namespace that contains Glusterfs endpoint. If this field is empty, the EndpointNamespace defaults to the same namespace as the bound PVC. More info: https://examples.k8s.io/volumes/glusterfs/README.md#create-a-pod")
    ...
    read_only: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("readOnly", "read_only"), serialization_alias="readOnly", description="readOnly here will force the Glusterfs volume to be mounted with read-only permissions. Defaults to false. More info: https://examples.k8s.io/volumes/glusterfs/README.md#create-a-pod")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "endpointsNamespace" not in obj and "endpoints_namespace" in obj:
            obj["endpointsNamespace"] = obj["endpoints_namespace"]
        obj.pop("endpoints_namespace", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("endpoints" if serialize else "endpoints"): _to_legacy_value(getattr(self, "endpoints", None), serialize),
            ("endpointsNamespace" if serialize else "endpoints_namespace"): _to_legacy_value(getattr(self, "endpoints_namespace", None), serialize),

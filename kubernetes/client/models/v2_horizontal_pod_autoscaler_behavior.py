...
class V2HorizontalPodAutoscalerBehavior(BaseModel):
    ...
    """ # noqa: E501
    scale_down: Optional[V2HPAScalingRules] = Field(default=None, validation_alias=AliasChoices("scaleDown", "scale_down"), serialization_alias="scaleDown")
    scale_up: Optional[V2HPAScalingRules] = Field(default=None, validation_alias=AliasChoices("scaleUp", "scale_up"), serialization_alias="scaleUp")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "scaleDown" not in obj and "scale_down" in obj:
            obj["scaleDown"] = obj["scale_down"]
        obj.pop("scale_down", None)
        if "scaleUp" not in obj and "scale_up" in obj:
            obj["scaleUp"] = obj["scale_up"]
        obj.pop("scale_up", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("scaleDown" if serialize else "scale_down"): _to_legacy_value(getattr(self, "scale_down", None), serialize),
            ("scaleUp" if serialize else "scale_up"): _to_legacy_value(getattr(self, "scale_up", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.scale_down:
            _dict['scaleDown'] = _to_openapi_value(self.scale_down)
        # override the default output from pydantic by calling `to_dict()` of scale_up
        if self.scale_up:
            _dict['scaleUp'] = _to_openapi_value(self.scale_up)
        return _dict

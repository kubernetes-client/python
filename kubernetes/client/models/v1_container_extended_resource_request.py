...
class V1ContainerExtendedResourceRequest(BaseModel):
    ...
    """ # noqa: E501
    container_name: StrictStr = Field(validation_alias=AliasChoices("containerName", "container_name"), serialization_alias="containerName", description="The name of the container requesting resources.")
    request_name: StrictStr = Field(validation_alias=AliasChoices("requestName", "request_name"), serialization_alias="requestName", description="The name of the request in the special ResourceClaim which corresponds to the extended resource.")
    resource_name: StrictStr = Field(validation_alias=AliasChoices("resourceName", "resource_name"), serialization_alias="resourceName", description="The name of the extended resource in that container which gets backed by DRA.")
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "containerName" not in obj and "container_name" in obj:
            obj["containerName"] = obj["container_name"]
        obj.pop("container_name", None)
        if "requestName" not in obj and "request_name" in obj:
            obj["requestName"] = obj["request_name"]
        obj.pop("request_name", None)
        if "resourceName" not in obj and "resource_name" in obj:
            obj["resourceName"] = obj["resource_name"]
        obj.pop("resource_name", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("containerName" if serialize else "container_name"): _to_legacy_value(getattr(self, "container_name", None), serialize),
            ("requestName" if serialize else "request_name"): _to_legacy_value(getattr(self, "request_name", None), serialize),
            ("resourceName" if serialize else "resource_name"): _to_legacy_value(getattr(self, "resource_name", None), serialize),
        }

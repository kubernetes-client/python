...
def get_resource_list_diff(
    resource_list_a: Optional[Mapping[str, str]],
    resource_list_b: Optional[Mapping[str, str]],
    *,
    ...
    Args:
        resource_list_a: Baseline ResourceList (treated as the *left* side),
            or ``None`` (treated as an empty mapping).
...
def resource_quotas_equal(
    ...
    """Return ``True`` when two ResourceQuota specs are fully identical.

    Compares the complete ``spec`` of both quotas:

    * ``spec.hard`` — compared semantically via :func:`get_resource_list_diff`
      so that unit-equivalent quantities (e.g. ``"1"`` and ``"1000m"``) are
      treated as equal, mirroring ``equality.Semantic.DeepEqual`` from the Go
      apimachinery package.
    * ``spec.scopes`` — compared as sets (order-independent).
    * ``spec.scope_selector`` — compared by equality of the serialised object.

    Status and metadata are intentionally excluded; only desired state matters
    for reconciler guards.
    ...
    Returns:
        ``True`` if both specs are fully identical, ``False`` otherwise.
    """
    spec_a = quota_a.spec
    spec_b = quota_b.spec

    hard_a = (spec_a and spec_a.hard) or {}
    hard_b = (spec_b and spec_b.hard) or {}
    if get_resource_list_diff(hard_a, hard_b):
        return False

    scopes_a = set(spec_a.scopes or []) if spec_a else set()
    scopes_b = set(spec_b.scopes or []) if spec_b else set()
    if scopes_a != scopes_b:
        return False

    selector_a = (spec_a and spec_a.scope_selector) or None
    selector_b = (spec_b and spec_b.scope_selector) or None
